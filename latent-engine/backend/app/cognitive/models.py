...
class OwnershipReport(DomainReport):
    owners: List[dict] = dataclasses.field(default_factory=list)
    critical_paths: List[dict] = dataclasses.field(default_factory=list)
    orphan_modules: List[str] = dataclasses.field(default_factory=list)
    ownership_conflicts: List[str] = dataclasses.field(default_factory=list)
    confidence: float = 1.0

@dataclasses.dataclass(frozen=True)
class BusFactorReport(DomainReport):
    overall_bus_factor: int = 0
    highest_risk_modules: List[dict] = dataclasses.field(default_factory=list)
    lowest_bus_factor_modules: List[dict] = dataclasses.field(default_factory=list)
...
class KnowledgeGraphReport(DomainReport):
    ...
    clusters: int = 0
    communities: int = 0
    central_nodes: List[str] = dataclasses.field(default_factory=list)
    critical_paths: List[str] = dataclasses.field(default_factory=list)
    dependency_summary: dict = dataclasses.field(default_factory=dict)
