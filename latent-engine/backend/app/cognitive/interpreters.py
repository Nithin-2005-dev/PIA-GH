...
class OwnershipInterpreter(CapabilityInterpreter):
    def interpret(self, raw_data: Any, contract: Any) -> DomainReport:
        d = asdict(raw_data) if is_dataclass(raw_data) else (raw_data if isinstance(raw_data, dict) else {})
        ownership_entries = list(d.get("ownership", []))
        formatted_owners = []
        for entry in ownership_entries:
            if isinstance(entry, dict):
                formatted_owners.append({"subject": entry.get("subject"), "level": entry.get("ownership_level")})
            else:
                formatted_owners.append({"subject": getattr(entry, "subject", "unknown"), "level": getattr(entry, "ownership_level", "unknown")})
        
        return OwnershipReport(
            owners=formatted_owners,
            executive_summary=ExecutiveSummaryBuilder.build("Ownership", raw_data)
        )
...
class BusFactorInterpreter(CapabilityInterpreter):
    def interpret(self, raw_data: Any, contract: Any) -> DomainReport:
        ...
        min_bf = 0
        if bus_factors:
            if isinstance(bus_factors[0], dict):
                min_bf = min(bf.get("bus_factor", 0) for bf in bus_factors)
            else:
                min_bf = min(getattr(bf, "bus_factor", 0) for bf in bus_factors)
                
        return BusFactorReport(
            overall_bus_factor=int(min_bf),
            executive_summary=ExecutiveSummaryBuilder.build("BusFactor", raw_data)
        )
...
class KnowledgeGraphInterpreter(CapabilityInterpreter):
    def interpret(self, raw_data: Any, contract: Any) -> DomainReport:
        nodes = len(raw_data.nodes) if hasattr(raw_data, "nodes") else 0
        return KnowledgeGraphReport(
            nodes=nodes,
            executive_summary=ExecutiveSummaryBuilder.build("KnowledgeGraph", raw_data)
        )
